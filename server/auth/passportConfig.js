...
/**
 * Normalize a desired username:
 * - Lowercase
 * - Keep letters, digits, underscore, dot
 * - Collapse underscores, trim to max length
 */
...
/**
 * Generate a unique username based on a desired base.
 * Adds a random numeric suffix if needed.
 */
async function getAvailableUsername(desiredBase) {
  const MAX_LEN = 30;
  ...
  // Try a few random suffixes
  for (let i = 0; i < 10; i++) {
    const suffix = Math.floor(Math.random() * 10000); // 0-9999
